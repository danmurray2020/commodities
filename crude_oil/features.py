"""Feature engineering for Crude Oil price prediction."""
...
def add_price_features(df: pd.DataFrame, price_col: str = "crude_oil_close") -> pd.DataFrame:
    df = df.copy()
    price = df[price_col]
...
def build_target(df, price_col="crude_oil_close", horizon=63):
    df = df.copy()
...
def prepare_dataset(csv_path=str(DATA_DIR / "combined_features.csv"), horizon=63):
    ...
    df = add_regime_features(df, price_col="crude_oil_close")
    df = build_target(df, horizon=horizon)
    df = df.ffill()
    df = df.dropna()
    exclude = {"crude_oil_close", "Open", "High", "Low", "Volume", "target_return", "target_direction"}
