"""Fetch Crude Oil futures data and supplementary market data."""
...
def fetch_prices(ticker: str = "CL=F", period: str = "10y") -> pd.DataFrame:
...
def main():
    ...
    prices = fetch_prices()
    prices.to_csv(DATA_DIR / "crude_oil_prices.csv")

    supplementary = fetch_supplementary_data()
    combined = prices[["Close"]].rename(columns={"Close": "crude_oil_close"})
