"""Fetch copper futures data and supplementary market data."""
...
def fetch_prices(ticker: str = "HG=F", period: str = "10y") -> pd.DataFrame:
...
def main():
    ...
    prices = fetch_prices()
    prices.to_csv(DATA_DIR / "copper_prices.csv")

    supplementary = fetch_supplementary_data()
    combined = prices[["Close"]].rename(columns={"Close": "copper_close"})
