"""Fetch Gasoline futures data and supplementary market data."""
...
def fetch_prices(ticker: str = "RB=F", period: str = "10y") -> pd.DataFrame:
...
def main():
    ...
    prices = fetch_prices()
    prices.to_csv(DATA_DIR / "gasoline_prices.csv")

    supplementary = fetch_supplementary_data()
    combined = prices[["Close"]].rename(columns={"Close": "gasoline_close"})
