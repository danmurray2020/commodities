"""Feature engineering for wheat price prediction."""
...
def add_price_features(df: pd.DataFrame, price_col: str = "wheat_close") -> pd.DataFrame:
    df = df.copy()
    price = df[price_col]
...
def build_target(df, price_col="wheat_close", horizon=63):
    df = df.copy()
...
def merge_cot_data(df, cot_path=str(DATA_DIR / "wheat_cot.csv")):
...
def prepare_dataset(csv_path=str(DATA_DIR / "combined_features.csv"), horizon=63,
    ...
    df = add_regime_features(df, price_col="wheat_close")
    ...
    df = df.ffill()
    df = df.dropna()
    exclude = {"wheat_close", "Open", "High", "Low", "Volume", "target_return", "target_direction"}
