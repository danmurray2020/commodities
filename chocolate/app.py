"""Web dashboard for cocoa price predictions."""
...
def get_prediction():
    ...
    current_price = float(latest["cocoa_close"].values[0])
    ...
    prices_2y = prices_df.loc[prices_df.index >= cutoff, "cocoa_close"]
...
<head>
    ...
    <title>Cocoa Futures Dashboard</title>
...
<div class="container">
    ...
    <div class="disclaimer"><strong>Disclaimer:</strong> This model is for educational and research purposes only. It is not financial advice. Cocoa futures are volatile and past performance does not guarantee future results.</div>
</div>
<script>
    ...
    new Chart(ctx, { type: 'line', data: { labels: allDates, datasets: [
        { label: 'Cocoa Price (USD/mt)', data: actualPrices, borderColor: '#f59e0b', backgroundColor: 'rgba(245,158,11,0.08)', fill: true, tension: 0.1, pointRadius: 0, borderWidth: 2 },
...
if __name__ == "__main__":
    print("Starting Cocoa Futures Dashboard...")
    print("Open http://localhost:8051 in your browser")
    app.run(host="127.0.0.1", debug=False, port=8051)
