"""Fetch CFTC COT data for natural gas futures."""
...
def extract_cot(df):
    ng = df[df["CFTC Commodity Code"] == NATGAS_CODE].copy()
    ng["Date"] = pd.to_datetime(ng["As of Date in Form YYYY-MM-DD"])
    ng = ng.sort_values("Date")
    cols = {
        "Date": "Date",
    ...
    result = ng[list(cols.keys())].rename(columns=cols)
...
def main():
    ...
    cot.to_csv(DATA_DIR / "natgas_cot.csv")
    print(f"Saved {len(cot)} COT records to {DATA_DIR / 'natgas_cot.csv'}")
    print(f"Latest: net_spec={cot.iloc[-1]['cot_net_spec']:.0f}")
