"""Web dashboard for natgas price predictions."""
...
def get_prediction():
    ...
    current_price = float(latest["natgas_close"].values[0])
    ...
    target_date = (latest.index[0] + timedelta(days=63)).strftime("%Y-%m-%d")
    ...
    prices_2y = prices_df.loc[prices_df.index >= cutoff, "natgas_close"]
...
<head>
    ...
    <title>Natural Gas Futures Dashboard</title>
...
<div class="container">
    <header>
        <h1>Natural Gas Futures Dashboard</h1>
    ...
    <div class="disclaimer"><strong>Disclaimer:</strong> This model is for educational and research purposes only. It is not financial advice. Natural Gas futures are volatile and past performance does not guarantee future results.</div>
</div>
<script>
    ...
    new Chart(ctx, { type: 'line', data: { labels: allDates, datasets: [
        { label: 'Natural Gas Price (USD/mt)', data: actualPrices, borderColor: '#ef4444', backgroundColor: 'rgba(245,158,11,0.08)', fill: true, tension: 0.1, pointRadius: 0, borderWidth: 2 },
...
if __name__ == "__main__":
    print("Starting Natural Gas Futures Dashboard...")
    print("Open http://localhost:8053 in your browser")
    app.run(host="127.0.0.1", debug=False, port=8053)
