"""Fetch Corn futures data and supplementary market data."""
...
def fetch_prices(ticker: str = "ZC=F", period: str = "10y") -> pd.DataFrame:
...
def main():
    ...
    prices = fetch_prices()
    prices.to_csv(DATA_DIR / "corn_prices.csv")

    supplementary = fetch_supplementary_data()
    combined = prices[["Close"]].rename(columns={"Close": "corn_close"})
