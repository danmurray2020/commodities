"""Fetch CFTC Commitment of Traders data for sugar futures."""
...
def extract_cot(df: pd.DataFrame) -> pd.DataFrame:
    sugar = df[df["CFTC Commodity Code"] == SUGAR_CODE].copy()
    sugar["Date"] = pd.to_datetime(sugar["As of Date in Form YYYY-MM-DD"])
    sugar = sugar.sort_values("Date")
    cols = {
        "Date": "Date",
    ...
    result = sugar[list(cols.keys())].rename(columns=cols)
...
def main():
    ...
    for year in range(2017, 2027):
        try:
            all_dfs.append(download_cot_zip(YEAR_URL_TEMPLATE.format(year=year)))
    ...
    cot.to_csv(DATA_DIR / "sugar_cot.csv")
    print(f"\nSaved {len(cot)} COT records to {DATA_DIR / 'sugar_cot.csv'}")
    print(f"Date range: {cot.index.min()} to {cot.index.max()}")
    print(f"\nLatest: net_spec={cot.iloc[-1]['cot_net_spec']:.0f}, spec_ratio={cot.iloc[-1]['cot_spec_ratio']:.3f}")
