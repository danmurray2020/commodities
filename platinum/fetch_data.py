"""Fetch Platinum futures data and supplementary market data."""
...
def fetch_prices(ticker: str = "PL=F", period: str = "10y") -> pd.DataFrame:
...
def main():
    ...
    prices = fetch_prices()
    prices.to_csv(DATA_DIR / "platinum_prices.csv")

    supplementary = fetch_supplementary_data()
    combined = prices[["Close"]].rename(columns={"Close": "platinum_close"})
